...
def _onstart_script(tier: str) -> str:
    ...
    if spec.get("needs_h3_stack"):
        lines += [
            ...
            # pin above) makes DynamicVRAM keep that much free and stream
            # weights instead: the same job then samples at 21.8GB with no
            # measurable per-step cost. Measured on a 110GB-RAM box; the
            # streaming reads the DiT back from pinned RAM, so it rides with
            # the smart-memory branch and is left off the <48GB path.
            'if [ "$RAM_GB" -ge 48 ];'
