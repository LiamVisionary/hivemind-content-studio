...
class TestTaskService(unittest.TestCase):
    def test_is_task_busy_covers_generation(self):
        """删除入口必须识别视频生成的活跃状态。

        Publishing no longer makes a task busy: render-time cross-posting was
        removed, so nothing reads the task directory after generation ends.
        """
        self.assertTrue(tm.is_task_busy({"state": tm.const.TASK_STATE_PROCESSING}))
        self.assertFalse(tm.is_task_busy({"state": tm.const.TASK_STATE_COMPLETE}))
    ...
    def test_start_completes_video(self):
        """
