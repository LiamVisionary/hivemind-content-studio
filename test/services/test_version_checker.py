...
class TestProjectVersionMetadata(unittest.TestCase):
    """
    在 fork 中，``app.__version__`` 表示内嵌的 MoneyPrinterTurbo 引擎版本，
    它被 ``version_checker`` 拿去和上游 GitHub release 比较，用来提示上游有新版本；
    ``project.version`` 则是本仓库自己的版本号。两者不再相等，
    因此这里断言引擎版本与记录在 ``[tool.hivemind.upstream]`` 的同步点一致，
    避免合并上游后忘记更新同步点。
    """

    def test_runtime_version_matches_recorded_upstream_sync_point(self):
        project_root = Path(__file__).resolve().parents[2]
        pyproject = tomllib.loads(
            (project_root / "pyproject.toml").read_text(encoding="utf-8")
        )
        upstream = pyproject["tool"]["hivemind"]["upstream"]

        self.assertEqual(__version__, upstream["moneyprinterturbo_version"])
