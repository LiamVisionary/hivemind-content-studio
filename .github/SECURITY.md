...
| 8788 | Canvas / gateway frontend (`packages/media-gateway/server.js`) | `127.0.0.1` (`HOST`) | **Gateway token or owner session cookie** (`lib/canvas-gate.js`). It is the iframe the studio frames and a ComfyUI HTTP/WS proxy, and it attaches the gateway token to everything it forwards, so it authenticates its own callers. `GET /healthz` answers unauthenticated — liveness only, so a supervisor can see the child before anyone signs in. |
| 8794 | Local-inference bridge (`hosted-server.js`) | `127.0.0.1` (`OGA_HOST`) | **Gateway token or owner session cookie** — the same `lib/canvas-gate.js` as 8788, because it holds the same token and spends the owner's Civitai key. `GET /health` and `/healthz` answer unauthenticated; everything else, including the served studio build, presents a credential. Loopback binding is not a credential: it stops nothing a browser on another site can aim at 127.0.0.1. |
