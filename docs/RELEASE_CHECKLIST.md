...
.venv/bin/python scripts/check_updater_config.py
...
2. Dispatch **Release desktop (build only)** with the version (semver, no
   ...
   `tauri.conf.json`, and a desktop dependency set that carries the Streamlit
   stack. The macOS lane additionally refuses a staged `ffmpeg` whose licence
   `docs/notices.json` does not record.
...
4. Smoke-test the DMG on a **second** machine — install, launch, sign in, one
   ...
   confirm a native save sheet appears and writes the file**
   ([`RELEASE.md`](RELEASE.md) §2.5 — the webview's own download path does not
   exist in a Tauri window), quit, and check again that no ComfyUI process was
   killed on quit.
