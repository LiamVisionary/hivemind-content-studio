...
11. Studio-started video generations are recorded automatically, next to run attempts, in the same telemetry (`/api/telemetry/generations`, the hub's Activity page, the CLI and MCP surfaces above). The ledger is `studio-generations.jsonl` beside the run store: one row per status change (`started`, `completed`, `failed`, `cancelled`) carrying the attempt id, workflow id, "Run on" pin, duration, the output's byte size (so an empty output is visible as one), LoRAs as a count plus the base model family they target, and on failure a machine-safe classification — a code such as `missing_node_type`, `out_of_memory` or `lane_unreachable` and, where the backend named one, the ComfyUI node class it refused. Prompts, media, file names, LoRA names and error text are never written. The Media Studio MCP produces the classification (`failure` on every refusal receipt, redacted or not, with a sentence that names the fix) and writes one `[media-studio-mcp] telemetry {...}` line per generate outcome to its stderr — the supervisor keeps that stream in `media-studio-mcp.log` / `hivemind-node-services.log` under the private root — in both privacy modes, so a refusal is findable without the client that saw it.
12. A registered workflow is checked against the lane that will run it BEFORE Generate (`GET /api/workflows/dependencies?workflow_id=&run_on=` on the media gateway, `/local-ai/workflow-dependencies` through the studio). The gateway reads the workflow's own API graph, asks the lane `/object_info/<class>` for every node class and every loader input's model list, and `/system_stats` for the accelerator, then names each missing node pack or model file with its source: `custom_node_dependencies` / `model_dependencies` on the registry entry (inherited through `inherits`), then ComfyUI-Manager's extension-node-map; `core_node_classes` marks nodes that ship inside a newer ComfyUI (reported as "update ComfyUI", never installed); `hardware.accelerator` blocks every install on a lane whose card cannot run the model and points at Machines. The studio opens the dependency prompt with one live progress row per item, starts every installable item at once (`POST .../install`, jobs under the same records as Civitai downloads, resumable `.part` downloads with size + SHA-256 checks, node packs cloned from github.com only and pinned), restarts the lane in place through ComfyUI-Manager (`POST .../restart`) once a node pack landed, and re-checks. Rented lanes are checked but not installed to: they are re-provisioned. The refusal path stays: a `missing_node_type` refusal answers with `remedy: install-dependencies`, and the failure callout's button opens the same prompt.
