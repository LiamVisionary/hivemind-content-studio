...
10. Dry-run publishing. Call `social_publish_rails` to see where a post can go; `prepare_social_publish` defaults to `provider: "auto"` (HivemindOS when it is open on this machine, then hosted publishing, then the owner's own keys). A draft prepared for `hivemindos` is never published from here: call `handoff_social_publish`, which queues a review suggestion in HivemindOS Socials and publishes nothing, then `sync_social_publish` to pull state and numbers back. The hand-off writes an unencrypted copy of the file and is refused unless the run is approved or its persona is reviewed in HivemindOS. Live publishing on any other rail still requires the approved manifest, enabled live-publish policy, and explicit outward-action gate.
10a. For a recurring character, use `save_content_persona`, gather trend notes with whatever research tools you have, then `plan_persona_day` and `start_persona_day`. Each run stops for generation and for a person's review. The autopilot never publishes, and a persona is labelled as AI unless the owner turned that off.
