...
# This shell is archived under archive/moneyprinterturbo/, and the engine it
# drives (`app/`) stays at the repository root — three directories up from the
# `webui` folder rather than one. See archive/moneyprinterturbo/README.md.
webui_dir = os.path.dirname(os.path.realpath(__file__))
root_dir = os.path.dirname(os.path.dirname(os.path.dirname(webui_dir)))
...
def _render_audio_settings(panel, params):
    ...
    with panel:
        with st.container(border=True):
            ...
            elif selected_tts_server == "localtts":
                # Hivemind 本地 TTS 服务：音色列表来自 /v1/voices，取不到时回退内置列表。
