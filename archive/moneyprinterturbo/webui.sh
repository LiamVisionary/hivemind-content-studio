...
# This shell is archived under archive/moneyprinterturbo/. It still drives the
# engine in `app/`, which lives at the repository root two directories up, so
# that is what goes on PYTHONPATH and what holds .venv / uv.lock.
CURRENT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$CURRENT_DIR/../.." && pwd)
...
if [ -x "$ROOT_DIR/.venv/bin/python" ]; then
  PORT_CHECK_CMD="$ROOT_DIR/.venv/bin/python"
  set -- "$ROOT_DIR/.venv/bin/python" -m streamlit
