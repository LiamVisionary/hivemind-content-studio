@echo off
setlocal
rem This shell is archived under archive\moneyprinterturbo\. The engine it drives
rem (app\) still lives at the repository root, two directories up from here.
set "SCRIPT_DIR=%~dp0"
...
%STREAMLIT_CMD% run "%SCRIPT_DIR%webui\Main.py" --server.address=%MPT_WEBUI_HOST% --server.port=%MPT_WEBUI_PORT% --browser.serverAddress=%MPT_WEBUI_HOST% --browser.gatherUsageStats=False --client.toolbarMode=minimal --logger.hideWelcomeMessage=True --server.showEmailPrompt=False --server.enableCORS=True
