...
`./scripts/build.sh` wraps `xcodebuild`, ensures the Metal toolchain is available (`metal`/`metallib`), and applies the same non-interactive plugin settings used in CI.
