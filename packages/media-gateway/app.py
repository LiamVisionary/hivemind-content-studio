...
RESTORE_ROOT = GATEWAY_STATE_DIR / "restore"
...
def assemble_restore_master(project, directory, *, output, source_for_audio=None):
    ...
    segments = []
    try:
        for position, step in enumerate(video_restore.assembly_steps(plan)):
            ...
            if step["kind"] == "trim":
                ...
                _run_ffmpeg(
                    [
                        ...
                        "-vf", video_restore.trim_filter(step["start"], step["length"]),
                ...
                _run_ffmpeg(
                    [
                        ...
                        "-filter_complex", video_restore.blend_filter_complex(step),
        ...
        filters = video_restore.finishing_filters(finish, width=plan["width"], height=plan["height"])
        ...
        args += video_restore.master_encode_args(finish)
...
def lane_restore_capability(lane_url, timeout=8.0):
    ...
    capability = {
        ...
        "models": [],
        "attention_modes": [],
        "missing": list(video_restore.REQUIRED_NODE_CLASSES),
        "tensorrt": {"available": False, "reason": "not asked yet"},
    }
    try:
        with urlopen(f"{lane_url}/object_info", timeout=timeout) as response:
            info = json.loads(response.read().decode("utf-8") or "{}")
    except Exception:
        return capability
    if not isinstance(info, dict):
        return capability
    capability["missing"] = [name for name in video_restore.REQUIRED_NODE_CLASSES if name not in info]
    ...
    capability.update({
        ...
        "tensorrt": lane_tensorrt_capability(lane_url, has_node=video_restore.TENSORRT_NODE_CLASS in info),
    })
    return capability
...
def lane_tensorrt_capability(lane_url, *, has_node, timeout=8.0):
    ...
    try:
        with urlopen(f"{lane_url}/hivemind/seedvr2-trt", timeout=timeout) as response:
            payload = json.loads(response.read().decode("utf-8") or "{}")
    except Exception as exc:
        return {
            "available": False,
...
def _restore_lane_devices(capability, options):
    ...
    offload = video_restore.resolve_offload_device(
        capability.get("offload_devices") or [],
        str(options.get("offload_device") or ""),
        device=device,
        cache_models=bool_option(options, "cache_models", True),
    )
...
def _restore_chunk_on_lane(project, chunk, *, source_name, lane_name, lane_url, capability, job_id):
    ...
    remote = comfy_lane_is_remote(lane_name)
    sink = video_restore.SINK_CLIP if remote else video_restore.SINK_FRAMES
    device, offload = _restore_lane_devices(capability, options)
    policy = video_restore.tensorrt_policy(
        plan, capability, requested=bool_option(options, "tensorrt", True),
    )
    graph = video_restore.build_restore_graph(
        ...
        cache_models=bool_option(options, "cache_models", True),
        tiled_vae=bool_option(options, "tiled_vae", False),
        tile_size=int_option(options, "tile_size", 1024, 128, 4096),
        # Refused rather than passed through: it crashes chunk two.
        torch_compile=(bool_option(options, "torch_compile", False)
                       and video_restore.torch_compile_supported()[0]),
        tensorrt=policy["enabled"],
        tensorrt_may_build=policy["may_build"],
        tensorrt_fp16=bool_option(options, "tensorrt_fp16", True),
    )
    ...
    try:
        queued = json.loads(urlopen(request, timeout=60).read().decode("utf-8"))
    except HTTPError as exc:
        detail = exc.read().decode("utf-8", errors="replace")
    ...
    if remote:
        record_comfy_prompt_route(prompt_id, lane_name, pushed_inputs=pushed_inputs, client_id=client_id)
        route = watch_remote_comfy_prompt(prompt_id, timeout_seconds=RESTORE_CHUNK_TIMEOUT_SECONDS) or {}
        if route.get("status") != "harvested":
    ...
    while time.monotonic() < deadline:
        if _restore_cancelled(project["id"]):
            try:
                urlopen(Request(f"{lane_url}/interrupt", data=b"", method="POST"), timeout=10).read()
            except Exception:
                pass
            raise RestoreCancelled("stopped")
        time.sleep(2)
        try:
            payload = urlopen(f"{lane_url}/history/{prompt_id}", timeout=15).read().decode("utf-8")
            data = json.loads(payload or "{}")
            if prompt_id in data:
                history = data[prompt_id]
                break
    ...
    for node_out in (history.get("outputs") or {}).values():
        for image in node_out.get("images") or []:
            resolved = resolve_comfy_temp_file(image.get("filename"), image.get("subfolder"))
...
def _restore_chunk_in_cloud(project, chunk, *, staged, destination, job_id):
    ...
    try:
        result = cloud_restore.restore_chunk(
            source=staged,
            destination=destination,
            request_body=video_restore.cloud_chunk_request(
    ...
    except cloud_restore.CloudRestoreError as exc:
...
def run_video_restore(job_id, video_path, options=None):
    ...
    started = now_iso()
    options = dict(options or {})
    project_id = safe_name(str(options.get("project_id") or "")) or f"r{uuid.uuid4().hex[:10]}"
    ...
    try:
        ...
        if manifest.is_file():
            project = video_restore.read_project(manifest)
        ...
        if project is None:
            ...
            if not video_restore.sink_supports_seams(sink):
                ...
                plan["seam_frames"] = 0
            project = video_restore.new_project(
        ...
        if sink == video_restore.SINK_CLOUD:
        ...
        rec["lane"] = lane_url
        video_restore.write_project(manifest, project)
        with jobs_lock:
            jobs[job_id] = rec
        ...
        for chunk in plan["chunks"]:
            ...
            try:
                if project["sink"] == video_restore.SINK_CLOUD:
            ...
            video_restore.write_project(manifest, project)
            rec["progress"] = video_restore.project_progress(project)
            with jobs_lock:
                jobs[job_id] = rec

        if not video_restore.sink_assembles_locally(project["sink"]):
            ...
            master = COMFY_OUTPUT_DIR / f"restore_{project_id}_{suffix}.mp4"
            mark_output_active(master)
            try:
                assemble_restore_master(project, directory, output=master, source_for_audio=source)
            finally:
                mark_output_inactive(master)
            ...
            rec.update({
                "status": "success",
                "finished_at": now_iso(),
                "outputs": encrypt_outputs([str(master)], job_id=job_id),
                "elapsed_seconds": round(time.monotonic() - t0, 2),
        ...
        if (project is not None and bool_option(options, "cache_models", True)
                and project.get("lane") != video_restore.CLOUD_LANE):
            # Nothing to free on the hosted lane: there is no machine holding
            # weights for us, which is the whole difference it is sold on.
            _free_restore_lane(project.get("lane") or "default", rec.get("lane") or COMFY_HTTP_DEFAULT)
    append_history(rec)
    with jobs_lock:
        jobs[job_id] = rec
...
def run_restore_finish(job_id, project_id, finish_options, assembled_path=None):
    ...
    try:
        ...
        master = COMFY_OUTPUT_DIR / f"restore_{project_id}_{suffix}_{uuid.uuid4().hex[:6]}.mp4"
        t0 = time.monotonic()
        mark_output_active(master)
        try:
...
def restore_pin_error(options):
    ...
    if not pin or pin == video_restore.CLOUD_LANE:
        return None
    try:
        comfy_lane_for_pin(pin)
...
def _cloud_restore_capability(health=None):
    ...
    return {
        ...
        "devices": [],
        "offload_devices": [],
        "models": list(video_restore.CLOUD_MODELS),
...
def _restore_sink_for_lane(lane_name):
    ...
    """
    if lane_name == video_restore.CLOUD_LANE:
        return video_restore.SINK_CLOUD
    return video_restore.SINK_CLIP if comfy_lane_is_remote(lane_name) else video_restore.SINK_FRAMES


def _restore_sink_word(sink):
    """The word a person would use for the machine behind a sink."""
    return {
        video_restore.SINK_CLIP: "rented",
        video_restore.SINK_CLOUD: "hosted",
    }.get(sink, "local")
...
def _resolve_restore_lane(plan, options):
    ...
    if pin == video_restore.CLOUD_LANE:
        ...
        return video_restore.CLOUD_LANE, "", capability
    lane_name = comfy_lane_for_pin(pin) if pin else None
    if lane_name is None:
        ...
        for candidate in list(COMFY_LANES):
            capability = lane_restore_capability(COMFY_LANES[candidate])
            if capability.get("available"):
                return candidate, COMFY_LANES[candidate], capability
        raise RuntimeError(
...
def restore_projects_summary(limit=50):
    ...
    for manifest in sorted(RESTORE_ROOT.glob("*/project.json")):
        ...
        projects.append({
            ...
            "progress": video_restore.project_progress(project),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/restore/projects":
            return self.send_json({
                "ok": True,
                "projects": restore_projects_summary(int(qs.get("limit", ["50"])[0] or 50)),
            })
        if parsed.path == "/api/restore/capabilities":
            ...
            lanes = []
            for lane_name, lane_url in sorted(COMFY_LANES.items()):
                capability = lane_restore_capability(lane_url)
                remote = comfy_lane_is_remote(lane_name)
                lanes.append({
                    "lane": lane_name,
            ...
            hosted = cloud_restore.status()
            lanes.append({
                "lane": video_restore.CLOUD_LANE,
                "remote": True,
                "paid": True,
            ...
            return self.send_json({
                "ok": True,
                "lanes": lanes,
                "models": list(video_restore.DIT_MODELS),
        ...
        if parsed.path.startswith("/api/restore/project/"):
            ...
            return self.send_json({
                ...
                "project": project,
                "progress": video_restore.project_progress(project),
                "resume_from": video_restore.first_unfinished_chunk(project),
                "assembly": video_restore.assembly_steps(project.get("plan") or {}),
            })
        if parsed.path.startswith("/api/restore/source/"):
            ...
            project_id = safe_name(parsed.path.rsplit("/", 1)[-1])
            try:
                source = restore_project_dir(project_id) / "source.mp4"
            except ValueError as exc:
                return self.send_json({"error": str(exc)}, 400)
            if not source.is_file():
    ...
    def do_POST(self):
        ...
        if parsed.path == "/api/restore":
            ...
            try:
                ...
                options["project_id"] = project_id
                pin_error = restore_pin_error(options)
                if pin_error:
                    if staged is not None:
                        staged.unlink(missing_ok=True)
                ...
                with jobs_lock:
                    jobs[job_id] = {
                        "id": job_id, "prompt": PRIVATE_PROMPT_LABEL, "status": "queued",
                        "created_at": now_iso(), "backend": "seedvr2-restore",
                    }
                threading.Thread(
                    target=run_video_restore, args=(job_id, staged, options), daemon=True,
                ).start()
                return self.send_json({
                    "id": job_id,
                    "status": "queued",
        ...
        if parsed.path == "/api/restore/plan":
            ...
            try:
                ...
                lane = None
                try:
                    lane_name, lane_url, capability = _resolve_restore_lane(plan, data.get("options") or {})
                    ...
                    lane = {
                        "lane": lane_name,
                        "remote": remote or lane_name == video_restore.CLOUD_LANE,
                        "paid": remote or lane_name == video_restore.CLOUD_LANE,
                        "assembles_here": video_restore.sink_assembles_locally(sink),
                        "models": capability.get("models") or [],
                        "devices": capability.get("devices") or [],
                        "attention_modes": capability.get("attention_modes") or [],
                    }
        ...
        if parsed.path == "/api/restore/finish":
            ...
            try:
                ...
                with jobs_lock:
                    jobs[job_id] = {
                        "id": job_id, "prompt": PRIVATE_PROMPT_LABEL, "status": "queued",
                        "created_at": now_iso(), "backend": "seedvr2-finish",
                    }
                threading.Thread(
                    target=run_restore_finish, args=(job_id, project_id, finish, assembled), daemon=True,
                ).start()
                return self.send_json({
        ...
        if parsed.path.startswith("/api/restore/cancel/"):
            ...
            request_restore_cancel(project_id)
