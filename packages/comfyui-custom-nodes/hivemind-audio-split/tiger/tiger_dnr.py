...
# (Apache-2.0), file look2hear/models/tiger_dnr.py.
#
...
from ..mps_pool import adaptive_avg_pool1d
...
class UConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        for fea in output:
            global_f = global_f + adaptive_avg_pool1d(
...
class TIGER(nn.Module):
    ...
    def pad_input(self, input, window, stride):
        ...
        if rest > 0:
            pad = torch.zeros(batch_size, rest).to(device=input.device, dtype=input.dtype)
            input = torch.cat([input, pad], 1)
        pad_aux = torch.zeros(batch_size, stride).to(device=input.device, dtype=input.dtype)
    ...
    def forward(self, input):
        ...
        spec = torch.stft(
            ...
            window=torch.hann_window(self.win).to(device=input.device, dtype=input.dtype),
            return_complex=True,
        )
...
class TIGERDNR(BaseModel):
    ...
    def wav_chunk_inference(self, model, mixture_tensor, target_length=12.0, hop_length=4.0, batch_size=1, n_tracks=3):
        ...
        if ignore > 0:
            zero_pad = torch.zeros(batch_mixture.shape[0], batch_mixture.shape[1], ignore).to(device=batch_mixture.device, dtype=batch_mixture.dtype)
            batch_mixture_pad = torch.cat([zero_pad, batch_mixture, zero_pad], -1)
        else:
            batch_mixture_pad = batch_mixture
        if target - hop > 0:
            hop_pad = torch.zeros(batch_mixture.shape[0], batch_mixture.shape[1], target-hop).to(device=batch_mixture.device, dtype=batch_mixture.dtype)
            batch_mixture_pad = torch.cat([hop_pad, batch_mixture_pad, hop_pad], -1)

        skip_idx = ignore + target - hop
        zero_pad = torch.zeros(batch_mixture.shape[0], batch_mixture.shape[1], session).to(device=batch_mixture.device, dtype=batch_mixture.dtype)
