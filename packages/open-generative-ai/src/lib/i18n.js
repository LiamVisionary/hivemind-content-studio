...
export const STRINGS = {
    ...
    'common.notConnected': NOT_CONNECTED,
    ...
    'place.hivemindosBlurb': 'Our own hosted models, billed to one balance of HivemindOS credits. Nothing to install and no app to keep open.',
    ...
    // ---- The Runs-on control ---------------------------------------------
    'runOn.label': 'Runs on',
    'runOn.accountModels': (count) => `${count} model${count === 1 ? '' : 's'} this account serves`,
    ...
    'runOn.rentedByTheHour': 'billed by the hour while it is rented',
    ...
    // The four tabs the list is segmented by. A tab is a HANDLE, not the bill's
    // full name: four of them share 360px, so "HivemindOS credits" and "Your
    // accounts" would each arrive truncated to a word and a half. The full
    // vocabulary is still said in the blurb under the strip, which is the line
    // that has room for it.
    'runOn.tabsLabel': 'Where the model runs',
    'runOn.tabHivemind': 'Hivemind',
    'runOn.tabRental': 'Rental',
    'runOn.tabLocal': 'Local',
    'runOn.tabAccounts': 'My accounts',
    // A rented GPU is still a property of This Mac everywhere that ROUTES — the
    // gateway sends the work there, the pin is per tab. The tab only separates
    ...
    // The account row, and the two sheets it opens.
    //
    // The disclaimer keys are the load-bearing ones. There are two of them
    // because there are two truths: an account nobody can reach but this
    // machine, and an account an email code brings back. Saying the first when
    // the second is true would be a lie that costs us trust; saying the second
    // when the first is true would be a lie that costs somebody their credits.
    'account.row': 'Account, credits and free usage',
    'account.openAccount': (name) => `${name} — account settings`,
    'account.openCredits': 'Credits and free usage',
    'account.notConnected': 'No account yet',
    'account.credits': (amount) => `${amount} credits`,
    'account.creditsUnknown': 'Credits unavailable',
    'account.freeMeter': (name) => `${name}, free today`,
    'account.freeLeft': (left, total) => `${left} of ${total} free`,
    'account.freePerDay': (total) => `${total} free a day`,
    'account.freeSpent': 'Free allowance used up',
    ...
    'account.unavailable': 'The studio could not say who this account is. It may be locked, or not running.',
    'account.unavailableShort': 'Account unavailable',
    'account.backupLocal': 'Only this machine can reach these credits. Lose it and they go with it.',
    'account.backedUpTo': (email) => `Backed up to ${email} — sign in with a code on any device.`,
    'account.backedUpShort': (email) => `Backed up to ${email}`,
    'account.notBackedUp': 'Not backed up yet',
    'account.noAccountYet': 'Add credits and we’ll make you one.',
    'account.emailHint': 'Attach an email and a six-digit code brings them back anywhere. No password, no HivemindOS app.',
    'account.emailLabel': 'Email address',
    'account.emailPlaceholder': 'you@example.com',
    'account.sendCode': 'Send code',
    'account.codeLabel': 'Six-digit code',
    'account.codePlaceholder': '000000',
    'account.codeSent': (email) => `Code sent to ${email}. It expires in a few minutes.`,
    'account.confirmCode': 'Confirm',
    'account.emailLinked': 'Email attached. These credits can be restored anywhere.',
    'account.changeEmail': 'Use a different address',
    'account.signInTitle': 'Already have credits?',
    'account.signInHint': 'Sign in with the email on your HivemindOS account. Anything bought here is folded into it.',
    'account.signIn': 'Sign in with email',
    'account.signedIn': 'Signed in. Your credits are on this machine now.',
    'account.signedInMerged': 'Signed in, and the credits already here were folded into that account.',
    'account.keyTitle': 'Account recovery key',
    'account.keyHint': 'Prefer to give no email? This key is your account — save it somewhere only you can read.',
    'account.keyReveal': 'Show recovery key',
    'account.keyWarning': 'Anyone who has this key can spend these credits, and it cannot be reissued. If you lose it with no email attached, the credits are gone.',
    'account.keyCopy': 'Copy key',
    'account.keyCopied': 'Recovery key copied.',
    'account.keyHide': 'Hide',
    'account.restoreTitle': 'Restore from a recovery key',
    'account.restoreHint': 'Paste a key from another install to bring its credits here.',
    'account.keyPlaceholder': 'hmos_credit_…',
    'account.restore': 'Restore these credits',
    'account.restored': 'Restored. That account’s credits are on this machine.',
    'account.ecosystem': 'One balance, everywhere',
    ...
    'credits.perDollar': (credits) => `${credits} credits per $1`,
    'credits.draftsLeft': (count, model) => `\u2248 ${count} ${model} drafts left`,
    'credits.modeTopUp': 'One-time top-up',
    'credits.modeMonthly': 'Monthly plan',
    'credits.betterRate': 'Better rate',
    'credits.amount': 'Amount',
    'credits.howToPay': 'How you\u2019ll pay',
    'credits.card': 'Card',
    'credits.cardHint': 'Pay with any card, on a page hosted by HivemindOS.',
    'credits.cardGo': 'Continue to checkout',
    'credits.crypto': 'USDC on Base',
    'credits.cryptoHint': 'Send from any wallet or exchange.',
    'credits.cryptoTag': '~1 min',
    'credits.cryptoUnavailable': 'Deposits are not switched on for this account.',
    'credits.cryptoPayer': 'The address you will send from',
    'credits.cryptoPayerHint': 'The transfer is matched against this address, so a transfer from a different one will not credit.',
    'credits.cryptoQuote': 'Get deposit address',
    'credits.cryptoSendTo': 'Send exactly this amount to',
    'credits.cryptoExpires': (minutes) => `expires in ${minutes} min`,
    'credits.cryptoExpired': 'This quote expired. Get a new deposit address.',
    'credits.cryptoFootnote': 'Paste the transaction hash after sending, and the credits land in about a minute.',
    'credits.cryptoHash': 'Transaction hash',
    'credits.addressPlaceholder': '0x\u2026',
    'credits.cryptoConfirm': 'I have sent it',
    'credits.cryptoSettled': (amount) => `${amount} of credits added.`,
    'credits.copy': 'Copy',
    'credits.wallet': 'HivemindOS wallet',
    'credits.walletHint': 'Pay from the wallet in your HivemindOS app. It will ask you to approve there.',
    'credits.walletTag': 'Approve in app',
    'credits.walletGo': 'Ask HivemindOS to pay',
    'credits.walletWaiting': 'Waiting for you to approve this in HivemindOS\u2026',
    'credits.walletApproved': 'Paid from your HivemindOS wallet.',
    'credits.walletRefused': 'That payment was not approved. Nothing was charged.',
    'credits.walletNoApp': 'The HivemindOS app did not answer. Open it and try again, or use another way to pay.',
    'credits.walletLocalOnly': 'Paying from the HivemindOS wallet works only on the machine the studio is running on.',
    'credits.walletOtherAccount': 'This studio is signed in to its own account, so the app\u2019s wallet would top up a different balance. Pay by card or USDC here, or sign in to the same account.',
    'credits.planCredits': (credits) => `${credits} / month`,
    'credits.planRate': (credits) => `${credits} per $1`,
    'credits.planNote': 'Credits arrive each month for as long as the plan runs.',
    'credits.plansUnavailable': 'The monthly plans could not be read just now.',
    'credits.choosePlan': 'Choose a plan',
    'credits.subscribeTo': (tier) => `Subscribe to ${tier}`,
    'credits.switchTo': (tier) => `Switch to ${tier}`,
    'credits.subscribed': (tier) => `${tier} is active`,
    'credits.managePlan': 'Manage plan',
    'credits.cancelTitle': 'End this plan?',
    'credits.cancelPlan': 'Cancel plan',
    'credits.cancelBody': 'Credits you have already been given stay. No more will be added, and the plan ends at the close of this period.',
    'credits.keepPlan': 'Keep it',
    'credits.cancelled': 'Plan cancelled.',
    'credits.honestyCard': 'Nothing is charged here \u2014 the checkout opens in a new tab.',
    'credits.honestyCrypto': 'Nothing is charged here \u2014 you send the transfer yourself.',
    'credits.honestyWallet': 'Nothing is charged here \u2014 the app asks you to approve it.',
    ...
    // Image Studio
    // Direction edits: point the eyes, move the sun. Both steer on a rendered
    // reference image rather than on words, so the copy talks about picking on
    // the picture, never about phrasing a prompt.
    'direction.eyesTitle': 'Point the eyes',
    'direction.sunTitle': 'Move the sun',
    'direction.eyesHint': 'Drag the red dot to where the eyes should look. Inside the frame is a look that stays in shot; outside it, the gaze leaves the picture.',
    'direction.sunHint': 'Drag the marker to where the sun should be. The camera stands at the bottom, the rim is the horizon and the middle is overhead.',
    'direction.eyesPickerLabel': 'Where the eyes look',
    'direction.sunPickerLabel': 'Where the sun is',
    'direction.eyesPosition': (x, y) => `${x}% across, ${y}% down`,
    'direction.referenceCaption': 'Sent to the model',
    'direction.strength': 'LoRA strength',
    'direction.strengthHint': 'Photographs and realistic art move at 0.5–1. Anime and other drawn styles usually need 1.25–1.5.',
    'direction.intensity': 'Sun strength',
    'direction.intensityHint': 'How hard the light is in the reference sphere.',
    'direction.extra': 'Extra words (optional)',
    'direction.extraEyesHint': 'Added after the trigger phrase. Naming the style helps on drawn and stylised pictures.',
    'direction.extraSunHint': 'Added after the trigger phrase. Say what the sky should do.',
    'direction.extraEyesPlaceholder': 'anime style',
    'direction.extraSunPlaceholder': 'add a clear blue sky',
    'direction.flatten': 'Flatten the existing light first',
    'direction.flattenHint': 'An extra pass that turns the picture overcast before the sun is placed. The LoRA lands cleanly only on a picture with no light direction of its own — turn this off if yours is already overcast.',
    'direction.run': 'Apply',
    'direction.running': 'Applying…',
    'direction.eyesModel': 'eyes pointed',
    'direction.sunModel': 'sun moved',
    'direction.started': 'Rendering the direction reference and running the edit…',
    'direction.done': 'Edited image added to the gallery.',
    'direction.failed': 'The direction edit failed',
    ...
    // The composer for a model that reads a picture and nothing else — no
    // prompt field upstream at all (AI Ghibli Style, the upscaler, the
    // background remover). See UploadOnlyComposer.
    'image.uploadOnlyEmpty': 'Drag and drop an image, or click to browse',
    'image.uploadOnlyReady': 'Ready — press Generate.',
    'image.uploadOnlyClear': 'Remove this picture',
    'image.uploadOnlyAlt': 'The picture this will work on',
    ...
    // Workflow dependency prompt (WorkflowDependencyPrompt.jsx)
    'deps.title': (name) => `${name} needs a few things first`,
    'deps.readyTitle': 'Ready to generate',
    'deps.readyBody': 'Everything this workflow needs is installed and the lane has loaded it.',
    'deps.intro': (count, size) => `${count} missing ${count === 1 ? 'item' : 'items'}${size ? ` (${size} to download)` : ''}. Installing what can be installed from here; each row shows its own progress and can be cancelled.`,
    'deps.cannotRunHere': 'This workflow cannot run on this machine',
    'deps.kind': (kind) => (kind === 'model' ? 'model file' : kind === 'custom_node' ? 'node pack' : kind === 'comfyui' ? 'ComfyUI' : String(kind || '')),
    'deps.install': 'Install',
    'deps.installing': 'Installing...',
    'deps.installFailed': 'The install could not be started.',
    'deps.cancelling': 'Cancelling...',
    'deps.checking': 'Checking the lane...',
    'deps.checkFailed': 'The lane could not be checked.',
    'deps.restartNeeded': 'A node pack was installed: the lane needs a restart to load it.',
    'deps.restartNow': 'Restart the lane',
    'deps.restarting': 'Restarting the lane so it loads the new node packs...',
    'deps.restartRefused': 'The lane could not be restarted from here. Restart the studio stack, then check again.',
    'deps.restartTimeout': 'The lane has not come back yet. Give it a moment, then check again.',
    'deps.installWhatIsMissing': 'Install what is missing',
    // What a studio says when it moved itself off a lane nobody chose in this
    // tab — a saved model that stopped being runnable, or the registry's own
    // default on a machine that cannot run it. Said in the composer, never in
    // a modal: nothing is blocked, so nothing should stop.
    'deps.movedTitle': (blocked, chosen) => `${blocked} is not set up to run here, so this tab is on ${chosen}.`,
    'deps.movedItems': (count, size) => `${count} missing ${count === 1 ? 'item' : 'items'}${size ? ` (${size} to download)` : ''}.`,
    'deps.setUpAnyway': (name) => `Set up ${name}`,
    'common.done': 'Done',
    ...
    'promptHelper.readingModelList': 'Reading the model list…',
    'promptHelper.freeingComfy': 'Freeing ComfyUI memory…',
    'promptHelper.unloading': 'Unloading…',
    'promptHelper.writePrompt': 'Write prompt',
    'promptHelper.refineTitle': "Rewrite this prompt into the model's perfect shape and fill in the craft details. Say what to change, or leave the box empty.",
    ...
    'promptHelper.loadingModel': (name) => `Loading ${name}…`,
    'promptHelper.yourIdea': 'Your idea',
    'promptHelper.mirrorsComposer': 'input · mirrors the composer',
    'promptHelper.writeFromThis': 'Write from this',
    'promptHelper.rewriteFromThis': 'Rewrite from this',
    'promptHelper.emptyTitle': 'Write a prompt from your idea',
    'promptHelper.emptyHint': "It will use your idea above, your cast, your references and the shot you're continuing from.",
    'promptHelper.staleDraft': 'Written from an earlier version of your idea.',
    'promptHelper.undo': 'Undo',
    'promptHelper.redo': 'Redo',
    'promptHelper.toUse': 'to use',
    'promptHelper.refinePlaceholder': 'What do you wanna change?',
    'promptHelper.tryColon': 'Try:',
    'promptHelper.suggestMoreDetail': 'add more detail',
    'promptHelper.suggestTighten': 'tighten it',
    'promptHelper.suggestAnotherShot': 'add another shot',
    'promptHelper.suggestSingleStill': 'make it a single still',
    'promptHelper.suggestTiming': 'fix the timing',
    'promptHelper.suggestMatchShot': (shot) => `match shot ${shot} harder`,
    'promptHelper.continuingShot': (shot) => `Continuing shot ${shot}`,
    'promptHelper.motionPinned': 'Motion context pinned',
    'promptHelper.shotSealed': 'The vault is locked in this tab, so this shot cannot be shown. Unlock the studio to see it.',
    'promptHelper.stopContinuing': 'Stop continuing',
    'promptHelper.continuationCarries': 'The new shot picks up exactly where this one ends — motion and room tone carry across the cut. ≈1s is re-rendered for the join, then trimmed off.',
    'promptHelper.shotSaid': (shot, said) => `Shot ${shot} said: “${said}”`,
    'promptHelper.readyNow': 'Ready now',
    'promptHelper.readyNowHint': 'no load, no wait',
    'promptHelper.loadsOnFirstUse': 'loads on first use',
    'promptHelper.lastUsed': 'Last used',
    ...
    'providers.oauthBlurb': 'OAuth stays inside HivemindOS — finish the sign-in in its tab and this updates by itself.',
    'providers.accountNamed': (label) => `${label} account`,
    'providers.countReady': (count) => `${count} ready`,
    'providers.countNeedSetup': (count) => `${count} need setup`,
    'providers.fallsBackTo': (target) => `falls back to ${target}`,
    'providers.groupAccounts': 'Accounts',
    'providers.groupPictureMotion': 'Picture & motion',
    'providers.groupSoundStock': 'Sound & stock',
    'providers.groupPipeline': 'Pipeline',
    'providers.groupOther': 'Other routes',
    ...
    'settings.developerHint': 'Repair tools for this machine’s own data. Off unless you need them.',
    'recover.explain': 'Paste a private key (PKCS#8 PEM). Every clip in this workspace is tested against it — stopping one step before any media exists — and the ones it opens are re-sealed to your vault, here in the browser. The key is never stored and never leaves this page.',
    'recover.scan': 'Scan library',
    'recover.chooseFile': 'Choose a .pem file',
    'recover.fingerprint': 'key',
    'recover.scanning': (done, total, opens) => `Testing ${done} of ${total}… ${opens} open so far`,
    'recover.scanned': (opens, refuses, skipped) => `${opens} open with this key · ${refuses} sealed to something else · ${skipped} not sealed`,
    'recover.resealN': (n) => `Re-seal ${n} to my vault`,
    'recover.resealing': (done, total) => `Re-sealing ${done} of ${total}…`,
    'recover.done': (ok, failed) => failed ? `${ok} re-sealed, ${failed} failed. The previous files were kept.` : `${ok} re-sealed to your vault. Reload to see them.`,
    'recover.noVaultKey': 'This workspace has no vault yet; create one before recovering.',
    ...
    'restorePanel.tiledVaeName': 'Tiled VAE',
    'restorePanel.tiledVae': 'Less memory, slower, and it can leave faint tile edges on flat gradients.',
    ...
    'inspo.partialFeed': (reason) => `${reason} These are the results that arrived — Load more picks up where it stopped.`,
