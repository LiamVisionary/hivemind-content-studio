// Ported from src/hivemind_content_studio/ui/studio.js — the hub SPA logic now
// lives inside the unified studio app. The host AppShell owns navigation and the
// topbar; this module owns the hub views (planner, canvas, models, runs,
// history, telemetry, providers) rendered from hubTemplates.js.
import {
  CREATE_VIEW, CANVAS_VIEW, MODELS_VIEW, RUNS_VIEW,
  HISTORY_VIEW, TELEMETRY_VIEW, PROVIDERS_VIEW, HUB_CHROME,
} from './hubTemplates.js';
...
const $ = (selector, scope = root || document) => scope.querySelector(selector);
const $$ = (selector, scope = root || document) => [...scope.querySelectorAll(selector)];
...
function selectNativeStudioMode(mode) {
  ...
  // The view element carries the mode for CSS (hub.css keys off
  // [data-view="create"][data-studio-mode=...]), so the button-highlight query
  // below MUST be scoped to the tab strip — a bare [data-studio-mode] also
  // matches the view itself and would add `is-active` to it, stacking the
  // create view on top of whichever hub view is actually open.
  createView.dataset.studioMode = selected;
  $$('#native-studio-modes [data-studio-mode]').forEach((button) => {
...
const HUB_VIEWS = ['create', 'canvas', 'models', 'runs', 'history', 'telemetry', 'providers'];
// View name → AppShell page id ('create' renders on the 'planner' page).
const hubPageForView = (view) => (view === 'create' ? 'planner' : view);

// Hub-internal navigation goes through the AppShell router so the rail,
// URL, and document title stay in sync. The router calls setActiveHubView.
...
export function setActiveHubView(view) {
  const selected = HUB_VIEWS.includes(view) ? view : 'create';
...
function renderCanvasHistoryFilters() {
  ...
  // Change-guarded: the periodic refresh re-runs this every 10s while the
  // History tab is open; rebuilding an unchanged <select> would close it
  // under the user's pointer.
  const key = JSON.stringify([state.canvasFormats, state.canvasModels, state.canvasFormat, state.canvasModel]);
  if (key === lastCanvasFilterKey) return;
  lastCanvasFilterKey = key;
...
// Padlock shown over a sealed thumbnail while its envelope downloads and
// decrypts in-page; the shackle swings open in the unlock animation.
const E2E_LOCK_SVG = `<svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path class="lock-shackle" d="M16 23v-7a8 8 0 0 1 16 0v7" stroke="currentColor" stroke-width="3.5" stroke-linecap="round"/>
    <g class="lock-body"><rect x="11" y="23" width="26" height="17" rx="4.5" fill="currentColor"/><circle cx="24" cy="30" r="2.6" fill="#0d0e12"/><rect x="22.9" y="31" width="2.2" height="4.6" rx="1.1" fill="#0d0e12"/></g>
  </svg>`;
...
function canvasHistoryEntryCard(entry) {
  ...
  const media = isVideo
    ...
    : `<span class="e2e-lock" aria-hidden="true">${E2E_LOCK_SVG}</span><img data-media-src="${esc(entry.media_url)}" alt="Private Canvas output" />`;
  ...
  return `<article class="canvas-history-card" data-canvas-history-id="${esc(entry.history_id)}">
    ...
    <div class="canvas-history-media"${isVideo ? '' : ' data-e2e-state="locked"'}>${media}</div>
...
function loadCanvasVideo(button) {
  ...
  void resolveMediaSrc(button.dataset.mediaUrl).then((resolved) => { video.src = resolved; video.load(); });
}

let lastPromptHistoryHtml = '';
...
function renderPromptHistory() {
  ...
  const html = `${canvas}${promptCards}`;
  // Change-guarded: skipping identical re-renders keeps already-decrypted
  // thumbnails (src set post-render by observeRenderedHistory) alive across
  // the periodic refresh instead of resetting them to unloaded envelopes.
  if (html === lastPromptHistoryHtml) return;
  lastPromptHistoryHtml = html;
  list.innerHTML = html;
  observeRenderedHistory();
}

// Decrypt-and-reveal choreography for sealed thumbnails: while the envelope
// downloads and decrypts in-page the card shows a breathing padlock (never the
// browser's broken-image glyph); once the pixels are ready the lock springs
// open for a second, then the image fades in. Media already decrypted this
// session (blob cache) skips the theater and shows immediately.
function revealHistoryMedia(image) {
  const container = image.closest('.canvas-history-media');
  const finish = (state) => { if (container) container.dataset.e2eState = state; };
  const cached = peekResolvedMediaSrc(image.dataset.mediaSrc);
  image.addEventListener('error', () => finish('failed'), { once: true });
  if (cached) {
    image.addEventListener('load', () => finish('unlocked'), { once: true });
    image.src = cached;
    return;
  }
  void resolveMediaSrc(image.dataset.mediaSrc).then((resolved) => {
    image.addEventListener('load', () => {
      finish('unlocking');
      window.setTimeout(() => finish('unlocked'), 1000);
    }, { once: true });
    image.src = resolved;
  });
}
...
function observeRenderedHistory() {
  ...
  historyMediaObserver = new IntersectionObserver((entries, observer) => {
    entries.forEach((entry) => {
      ...
      if (image.dataset.mediaSrc && !image.src) revealHistoryMedia(image);
...
function renderLoadedCanvasSetup(entry, setup) {
  ...
  // E2E-sealed outputs decrypt in-page to a blob URL; legacy media passes
  // through untouched (resolveMediaSrc is fail-open).
  void resolveMediaSrc(entry.media_url).then((resolved) => {
    const target = $(isVideo ? 'video' : 'img', $('.loaded-canvas-media', panel));
    if (target) target.src = resolved;
  });
...
async function loadRuns() {
  ...
  if (count) count.textContent = state.runs.length;
  renderRecentRun();
  refreshSimpleRunGenerationCards();
}
...
function bindEvents() {
  ...
  document.addEventListener('visibilitychange', () => { if (!document.hidden && root?.isConnected) refreshAll({ quiet: true }); });
}

// The AppShell topbar owns the status pill (#hub-api-status) and refresh
// button (#hub-refresh-button); the hub only reports into them if present.
...
async function refreshAll({ quiet = false } = {}) {
  ...
  try {
    ...
    if (!quiet) state.surfaces = await api('/api/surfaces');
    ...
    // another view appears without toggling away (renders above are
    // change-guarded, so unchanged data is a no-op).
    if ($('[data-view="history"]')?.classList.contains('is-active')) await loadPrompts({ quiet: true });
...
async function boot() {
  ...
  setInterval(() => { if (!document.hidden && root?.isConnected) refreshAll({ quiet: true }); }, 10000);
  setInterval(() => {
    if (!document.hidden && root?.isConnected && root.querySelector('.application-generation-card[data-status="running"]')) refreshSimpleRunGenerationCards();
  }, 1000);
}

// Build (once) and return the persistent hub root. The caller appends it to
// the content area and then calls setActiveHubView(view); DOM state and
// listeners survive page switches because the root is re-appended, not rebuilt.
export function ensureHub() {
  if (!root) {
    root = document.createElement('div');
    root.className = 'hub-root custom-scrollbar';
    root.style.height = '100%';
    root.style.overflowY = 'auto';
    root.innerHTML = [CREATE_VIEW, CANVAS_VIEW, MODELS_VIEW, RUNS_VIEW, HISTORY_VIEW, TELEMETRY_VIEW, PROVIDERS_VIEW, HUB_CHROME].join('\n');
  }
  if (!bootStarted) {
    bootStarted = true;
    void boot();
