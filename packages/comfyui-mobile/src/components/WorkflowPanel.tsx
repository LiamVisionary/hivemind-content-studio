...
const KREA2_CONVROT_PROMPT_ASSISTANT_WORKFLOW_FILENAME =
  "Krea2 Turbo ConvRot INT8 ASFP8 Bench Apple Silicon.json";
const KREA2_MULTI_LORA_STACK_NODE_TYPE = "Krea2MLXMultiLoRAStack";
const PROMPT_ASSISTANT_NODE_TYPE = "PromptAssistantGenerate";
const KREA2_GATED_REBALANCE_NODE_TYPE = "AzKrea2GatedRebalance";
...
function isKrea2ConvRotPromptAssistantWorkflowFilename(filename: string | null): boolean {
  if (!filename) return false;
  return filename.split("/").pop() === KREA2_CONVROT_PROMPT_ASSISTANT_WORKFLOW_FILENAME;
}
...
export const WorkflowPanel = memo(function WorkflowPanel({
  ...
  const repairedMissingKrea2PromptAssistantRef = useRef<string | null>(null);
  const repairedMissingKrea2GatedRebalanceRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (!workflow || !isKrea2ConvRotPromptAssistantWorkflowFilename(currentFilename)) return;
    if (workflow.nodes?.some((node) => node.type === PROMPT_ASSISTANT_NODE_TYPE)) return;
    if (repairedMissingKrea2PromptAssistantRef.current === currentFilename) return;

    repairedMissingKrea2PromptAssistantRef.current = currentFilename;
    void loadUserWorkflow(KREA2_CONVROT_PROMPT_ASSISTANT_WORKFLOW_FILENAME)
      .then((serverWorkflow) => {
        if (!serverWorkflow.nodes?.some((node) => node.type === PROMPT_ASSISTANT_NODE_TYPE)) {
          return;
        }
        const filename = currentFilename ?? KREA2_CONVROT_PROMPT_ASSISTANT_WORKFLOW_FILENAME;
        loadWorkflow(serverWorkflow, filename, {
          fresh: true,
          replaceActive: true,
          navigate: false,
      ...
      .catch((error) => {
        console.error("Failed to repair Krea2 ConvRot prompt assistant workflow from disk:", error);
        repairedMissingKrea2PromptAssistantRef.current = null;
      });
  }, [currentFilename, loadWorkflow, workflow]);

  useEffect(() => {
    if (!workflow || !isKrea2ConvRotPromptAssistantWorkflowFilename(currentFilename)) return;
    if (workflow.nodes?.some((node) => node.type === KREA2_GATED_REBALANCE_NODE_TYPE)) return;
    if (repairedMissingKrea2GatedRebalanceRef.current === currentFilename) return;

    repairedMissingKrea2GatedRebalanceRef.current = currentFilename;
    void loadUserWorkflow(KREA2_CONVROT_PROMPT_ASSISTANT_WORKFLOW_FILENAME)
      .then((serverWorkflow) => {
        if (!serverWorkflow.nodes?.some((node) => node.type === KREA2_GATED_REBALANCE_NODE_TYPE)) {
          return;
        }
        const filename = currentFilename ?? KREA2_CONVROT_PROMPT_ASSISTANT_WORKFLOW_FILENAME;
        loadWorkflow(serverWorkflow, filename, {
      ...
      .catch((error) => {
        console.error("Failed to repair Krea2 ConvRot gated rebalance workflow from disk:", error);
        repairedMissingKrea2GatedRebalanceRef.current = null;
      });
  }, [currentFilename, loadWorkflow, workflow]);
  ...
  useEffect(() => {
    ...
    const shouldRevealPromptAssistant =
      isAnimaPromptAssistantWorkflowFilename(currentFilename) ||
      isKrea2ConvRotPromptAssistantWorkflowFilename(currentFilename);
    const shouldRevealKrea2GatedRebalance =
      isKrea2ConvRotPromptAssistantWorkflowFilename(currentFilename);
    const collectNode = (node: WorkflowNode, subgraphId: string | null) => {
      if (
        !isMultiLoraStackNodeType(node.type) &&
        !(shouldRevealPromptAssistant && node.type === PROMPT_ASSISTANT_NODE_TYPE) &&
        !(shouldRevealKrea2GatedRebalance && node.type === KREA2_GATED_REBALANCE_NODE_TYPE)
      ) return;
  ...
  }, [currentFilename, hiddenItems, setItemHidden, workflow]);
