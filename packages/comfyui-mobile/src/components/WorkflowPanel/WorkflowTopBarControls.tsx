...
import { ModelLinkerDialog } from './ModelLinkerDialog';
import { CloudDownloadIcon, SaveAsIcon, SaveDiskIcon, TrashIcon, LogoutIcon, ReloadIcon, XMarkIcon } from '@/components/icons';
...
export function WorkflowTopBarControls() {
  ...
  const [modelLinkerOpen, setModelLinkerOpen] = useState(false);
  ...
  const handleWorkflowRelinked = (updatedWorkflow: typeof workflow) => {
    if (!updatedWorkflow) return;
    loadWorkflow(updatedWorkflow, currentFilename ?? undefined, {
      fresh: true,
      replaceActive: true,
      navigate: false,
      source: workflowSource ?? { type: 'other' },
      originalWorkflowOverride: originalWorkflow,
    });
  };
  ...
  return (
    <>
        ...
        >
          ...
          >
            ...
            <div className="max-h-[60vh] overflow-y-auto">
              ...
              <WorkflowActionButton
                icon={<CloudDownloadIcon className="w-5 h-5 text-slate-400" />}
                label="Relink missing models"
                description="Find installed or hardware-preferred model replacements"
                onClick={() => {
                  setActionsOpen(false);
                  setModelLinkerOpen(true);
                }}
                disabled={!workflow || loading}
              />
              <div className="border-t border-white/10" />
      ...
      {modelLinkerOpen && workflow && (
        <ModelLinkerDialog
          workflow={workflow}
          onClose={() => setModelLinkerOpen(false)}
          onWorkflowUpdated={handleWorkflowRelinked}
        />
      )}
