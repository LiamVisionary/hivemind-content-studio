...
import { deleteFile, moveFiles, createFolder, getUserImages, getRecursiveFolders, renameFile, getScreenPreviewUrl, getFileWorkflow } from '@/api/client';
...
export const OutputsPanel = memo(function OutputsPanel({ visible }: { visible: boolean }) {
  ...
  const findHistoryEntryForOutputItem = (item: ViewerImage) => {
    if (item.promptId) {
  ...
  const handleOutputsViewerCopyWorkflowDebug = async (item: ViewerImage) => {
    const historyEntry = findHistoryEntryForOutputItem(item);
    const resolvedWorkflowLoad = resolveViewerItemWorkflowLoad(
      item,
      historyWorkflowByFileId,
    );
    ...
    >
      ...
      >
       ...
       <MediaViewer
         ...
         onCopyWorkflowDebug={handleOutputsViewerCopyWorkflowDebug}
