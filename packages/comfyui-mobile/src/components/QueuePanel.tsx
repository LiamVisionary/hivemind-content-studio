...
export const QueuePanel = memo(function QueuePanel({ visible, onImageClick }: QueuePanelProps) {
  ...
  const handleCopyWorkflow = async ({
    ...
    const historyEntry = promptId
      ? history.find((entry) => entry.prompt_id === promptId) ?? null
      : null;
    const queueItem = promptId
      ? [...running, ...pending, ...completing]
          .find((candidate) => candidate.prompt_id === promptId) ?? null
      : null;
    ...
    const bundle = buildGenerationDebugBundle({
      ...
      historyEntry,
      queueItem,
      queueMetadata: promptId ? queueState.queueMetadata[promptId] ?? null : null,
      imageSources,
    });
    const text = JSON.stringify(bundle, null, 2);
    const copied = await copyTextToClipboard(text);
    setToastMessage(copied ? 'Copied workflow debug bundle' : 'Failed to copy');
    setTimeout(() => setToastMessage(null), 2000);
  };

  const handleDownload = async (src: string) => {
    await downloadImage(src, undefined, (downloadedSrc) => {
