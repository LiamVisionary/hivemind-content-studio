...
import { deleteFile, getFileWorkflow, type FileItem } from '@/api/client';
...
export function ImageViewer({ onClose }: ImageViewerProps) {
  ...
  const [copyDebugMessage, setCopyDebugMessage] = useState<string | null>(null);
  ...
  const findHistoryEntryForViewerItem = (item: ViewerImage) => {
    if (item.promptId) {
      const byPromptId = history.find((entry) => entry.prompt_id === item.promptId);
      if (byPromptId) return byPromptId;
    }
    const fileIdToMatch = item.file?.id;
    if (!fileIdToMatch) return null;
    return history.find((entry) =>
      entry.outputs.images.some((image) => getHistoryImageFileId(image) === fileIdToMatch),
    ) ?? null;
  };

  const handleCopyWorkflowDebug = async (item: ViewerImage) => {
    const historyEntry = findHistoryEntryForViewerItem(item);
    const resolvedWorkflowLoad = resolveViewerItemWorkflowLoad(
      item,
      historyWorkflowByFileId,
    );
    let workflowForDebug = resolvedWorkflowLoad?.workflow ?? item.workflow ?? null;
    const fileWorkflowLookup: Record<string, unknown> = {
      attempted: false,
      success: Boolean(workflowForDebug),
    };
    if (!workflowForDebug && item.file) {
      const assetSource = resolveFileSource(item.file);
      const filePath = resolveFilePath(item.file, assetSource);
      fileWorkflowLookup.attempted = true;
      fileWorkflowLookup.source = assetSource;
      fileWorkflowLookup.path = filePath;
      try {
        workflowForDebug = await getFileWorkflow(filePath, assetSource);
        fileWorkflowLookup.success = true;
      } catch (err) {
        fileWorkflowLookup.success = false;
        fileWorkflowLookup.error = err instanceof Error ? err.message : 'Failed to load workflow metadata';
      }
    }
    const resolvedPromptId = item.promptId ?? historyEntry?.prompt_id ?? null;
    const queueState = useQueueStore.getState();
    const queueItem = resolvedPromptId
      ? [...queueState.running, ...queueState.pending, ...queueState.completing]
          .find((candidate) => candidate.prompt_id === resolvedPromptId) ?? null
      : null;
    const bundle = buildGenerationDebugBundle({
      source: 'media-viewer',
      promptId: resolvedPromptId,
      status: historyEntry ? 'done' : queueItem?.prompt_id ? 'running' : null,
      workflow: workflowForDebug,
      historyEntry,
      queueItem,
      queueMetadata: resolvedPromptId ? queueState.queueMetadata[resolvedPromptId] ?? null : null,
      imageSources: item.src ? [item.src] : [],
      fileId: item.file?.id ?? null,
      filename: item.filename ?? item.file?.name ?? null,
    });
    bundle.fileWorkflowLookup = fileWorkflowLookup;
    const copied = await copyTextToClipboard(JSON.stringify(bundle, null, 2));
    setCopyDebugMessage(copied ? 'Copied workflow debug bundle' : 'Failed to copy');
    window.setTimeout(() => setCopyDebugMessage(null), copied ? 2200 : 3000);
  };
  ...
  return (
    <>
      <MediaViewer
        ...
        onCopyWorkflowDebug={handleCopyWorkflowDebug}
      ...
      <UseImageModal
        ...
        background="translucent"
      />
      {copyDebugMessage && createPortal(
        <div className="fixed top-20 left-1/2 z-[2300] -translate-x-1/2 rounded-lg border border-white/10 bg-slate-900/95 px-4 py-2 text-sm font-medium text-slate-100 shadow-lg">
          {copyDebugMessage}
        </div>,
        document.body
      )}
