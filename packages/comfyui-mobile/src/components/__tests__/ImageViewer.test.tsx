...
import { deleteFile, getFileWorkflow } from '@/api/client';
...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    queueState: {
      ...
      completing: [] as Array<{ prompt_id: string }>,
      queueMetadata: {} as Record<string, unknown>,
...
vi.mock('@/hooks/useQueue', () => {
  const useQueueStore = (selector: (state: typeof mocks.queueState) => unknown) =>
    selector(mocks.queueState);
  useQueueStore.getState = () => mocks.queueState;
...
describe('ImageViewer follow queue mode', () => {
  ...
  beforeEach(() => {
    ...
    mocks.queueState.completing = [];
    mocks.queueState.queueMetadata = {};
    ...
    vi.mocked(getFileWorkflow).mockReset();
  ...
  it('copies file workflow metadata for output-backed debug bundles without history workflow', async () => {
    mocks.workflowState.followQueue = false;
    const item = makeViewerImage(undefined);
    mocks.viewerState.viewerImages = [item];
    mocks.viewerState.viewerIndex = 0;
    vi.mocked(getFileWorkflow).mockResolvedValue({
      last_node_id: 2,
      last_link_id: 0,
      nodes: [
        {
    ...
    const props = mocks.mediaViewerProps.at(-1) as {
      onCopyWorkflowDebug?: (i: typeof item) => void;
    };
    await act(async () => {
      await props.onCopyWorkflowDebug?.(item);
    });

    expect(vi.mocked(getFileWorkflow)).toHaveBeenCalledWith('first.png', 'output');
    const copied = vi.mocked(copyTextToClipboard).mock.calls.at(-1)?.[0];
    expect(copied).toBeTypeOf('string');
    const bundle = JSON.parse(copied as string) as {
      workflow?: { nodes?: Array<{ type: string }> } | null;
      queue?: { hasWorkflow?: boolean };
      fileWorkflowLookup?: { attempted?: boolean; success?: boolean };
    };
    expect(bundle.queue?.hasWorkflow).toBe(true);
    expect(bundle.workflow?.nodes?.[0]?.type).toBe('EmptyLatentImage');
    expect(bundle.fileWorkflowLookup).toMatchObject({
      attempted: true,
      success: true,
    });
  });
