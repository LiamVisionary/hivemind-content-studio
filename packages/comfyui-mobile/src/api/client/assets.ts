...
import {
  ...
  extractWorkflowFromImageBytesAsync,
...
function viewUrlForAssetPath(filePath: string, source: AssetSource): string {
  const parts = filePath.split('/').filter(Boolean);
  const filename = parts.pop() || filePath;
  const subfolder = parts.join('/');
  const params = new URLSearchParams({ filename, type: source });
  if (subfolder) params.set('subfolder', subfolder);
  return `/view?${params.toString()}`;
}

async function getWorkflowFromDisplayedImage(
  filePath: string,
  source: AssetSource,
  signal?: AbortSignal,
): Promise<Workflow | null> {
  try {
    const response = await fetch(viewUrlForAssetPath(filePath, source), { signal });
    if (!response.ok) return null;
    const contentType = response.headers.get('content-type') || '';
    if (contentType && !contentType.toLowerCase().startsWith('image/')) return null;
    const bytes = new Uint8Array(await response.arrayBuffer());
    return extractWorkflowFromImageBytesAsync(bytes);
  } catch (err) {
...
export async function getFileWorkflow(
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    const workflowFromImage = await getWorkflowFromDisplayedImage(path, source, options?.signal);
    if (workflowFromImage) return workflowFromImage;
