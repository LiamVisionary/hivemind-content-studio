...
function App() {
  ...
  const openFollowQueueViewer = async () => {
    ...
    try {
      await useHistoryStore.getState().fetchHistory(INITIAL_HISTORY_PAGE_SIZE);
    } catch (err) {
      console.warn('Failed to refresh history before opening follow queue:', err);
    }

    // Read all stores fresh so the keydown handler (whose effect intentionally
    // doesn't re-register on every queue/history change) never opens with a
    // stale closed-over snapshot. Include live final outputs because native
    // jobs can finish before their authoritative history row is visible.
    const historyState = useHistoryStore.getState();
    const historyPromptIds = historyState.history
      .map((item) => item.prompt_id)
      .filter((promptId): promptId is string => Boolean(promptId));
    if (historyPromptIds.length > 0) {
      await useQueueStore.getState().fetchQueueMetadata(historyPromptIds).catch((err) => {
        console.warn('Failed to refresh queue metadata before opening follow queue:', err);
      });
    }
    const queueState = useQueueStore.getState();
    const workflowState = useWorkflowStore.getState();
    const allImages = buildFollowQueueViewerImages({
      history: historyState.history,
      livePromptOutputs: queueState.livePromptOutputs,
      localPromptOrder: queueState.localPromptOrder,
      promptToSession: workflowState.promptToSession,
      queueMetadata: queueState.queueMetadata,
      activeSessionId: workflowState.activeSessionId,
    });

    setViewerState({
      viewerImages: allImages,
      viewerIndex: allImages.length > 0 ? 0 : -1,
      viewerScale: 1,
      viewerTranslate: { x: 0, y: 0 },
      viewerOpen: true,
    });
  ...
  return (
    <div id="app-root" className="min-h-screen bg-slate-950">
      ...
      <BottomBar
        ...
        onOpenFollowQueue={() => { void openFollowQueueViewer(); }}
      />
