...
import { repairKrea2TurboPreLoraWorkflow } from "@/utils/krea2TurboPreLoraMigration";
...
interface LoadWorkflowOptions {
  ...
  originalWorkflowOverride?: Workflow | null;
}
...
function normalizeSessionInPlace(
  ...
  const krea2TurboMigration = repairKrea2TurboPreLoraWorkflow(s.workflow);
  if (krea2TurboMigration.changed) {
    s.workflow = krea2TurboMigration.workflow;
  }
  if (s.originalWorkflow) {
    ...
    const originalKrea2Migration = repairKrea2TurboPreLoraWorkflow(s.originalWorkflow);
    if (originalKrea2Migration.changed) {
      s.originalWorkflow = originalKrea2Migration.workflow;
    }
  }
  ...
  persist(
    (set, get) => {
      ...
      const loadWorkflow: WorkflowState["loadWorkflow"] = (
        ...
        const krea2TurboMigration = repairKrea2TurboPreLoraWorkflow(workflow);
        if (krea2TurboMigration.changed) {
          workflow = krea2TurboMigration.workflow;
          options = { ...options, fresh: true };
          if (filename) {
            const workflowForPersistence = validateAndNormalizeWorkflow(
              stripWorkflowClientMetadata(krea2TurboMigration.workflow),
            );
            void api.saveUserWorkflow(filename, workflowForPersistence).catch((error) => {
              console.error(
                'Failed to persist repaired Krea2 Turbo Pre-LoRA workflow:',
                error,
              );
            });
          }
        ...
        if (savedState) {
          ...
          set({
            ...
            originalWorkflow: structuredClone(
              options?.originalWorkflowOverride ?? restoredWorkflowWithHierarchicalKeys,
          ...
          set({
            ...
            originalWorkflow: structuredClone(
              options?.originalWorkflowOverride ?? normalizedWorkflowWithHierarchicalKeys,
            ),
            diffBaseWorkflow: null,
            lastEnqueuedWorkflow: null,
            scopeStack: [{ type: "root" as const }],
      ...
      const queueWorkflow: WorkflowState["queueWorkflow"] = async (
        ...
        try {
          ...
          for (let i = 0; i < count; i++) {
            ...
            const krea2TurboMigration = repairKrea2TurboPreLoraWorkflow(currentWorkflow);
            if (krea2TurboMigration.changed) {
              currentWorkflow = krea2TurboMigration.workflow;
              writeWorkflow(currentWorkflow);
            }
            ...
            let queuedPrompt = applyGenerationFilenamePrefixes(
              prompt,
              nextFilenameTimestamp(Date.now() + i),
            );
            if (useGenerationSettingsStore.getState().obfuscateSharedInputPaths) {
              const obfuscated = await obfuscateQueuedInputPaths(queuedPrompt, queuedWorkflow, nodeTypes);
