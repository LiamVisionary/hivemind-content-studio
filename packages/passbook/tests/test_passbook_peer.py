...
import passbook_peer as peer  # noqa: E402
...
def test_the_verdict_is_always_one_of_three(pair):
    identity = peer.peer_identity(pair[0], team="L7XLLTV3X7")
...
def test_a_socket_with_no_peer_is_unknown():
    ...
    try:
        identity = peer.peer_identity(lonely, team="L7XLLTV3X7")
    finally:
        lonely.close()
    assert identity["status"] == "unknown"
...
def test_the_requirement_names_the_team():
    text = peer.requirement_for_team("L7XLLTV3X7")
    assert "anchor apple generic" in text
    assert 'subject.OU] = "L7XLLTV3X7"' in text
